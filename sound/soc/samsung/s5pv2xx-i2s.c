...
#define S3C_CLKSRC_CLKAUDIO	(1<<10)
...
#define IISMOD_MASTER (0 << 11)
	...
	/*
	 * 1: IIS master or slave mode select
	 * refer to IISMOD[11]
	 */
	...
	/*
	 * 2: set Serial data format
	 * refer to pdf P1924: IISMOD[6:5]
	 */
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	case S3C64XX_CLKSRC_CDCLK:
		...
		case SND_SOC_CLOCK_OUT:
			iismod &= ~S3C64XX_IISMOD_CDCLKCON;	/* Use internal codec clock source */
			break;
		default:
			return -EINVAL;
		}
	...
	case S3C_CLKSRC_CLKAUDIO:
		/* 1: Select RCLK clock source = SCLK_AUDIO = EPLL */
		/*
		 * IISMOD[10]: set to 1
		 * Select RCLK(root clock==cdclk) clock source = SCLK_AUDIO = EPLL
		 * refer to P1876
		 */
		if (!i2s->master)
			return -EINVAL;
		iismod &= ~S3C_IISMOD_IMSMASK;	/* by the way to set iis master mode */
		iismod |= clk_id;	/* 1<<10: EPLL */

		/* 2: set epll */
		/* depend on [fs] to set the freq of [epll] */
...
int s5p_i2sv5_register_dai(struct device *dev, struct snd_soc_dai_driver *dai)
{
	...
	return snd_soc_register_component(dev, NULL, dai, 1);
	//return snd_soc_register_dai(dev, dai);
}

static int s3c64xx_iis_dev_probe(struct platform_device *pdev)
{
	printk("%s\n", __func__);
...
static int s3c64xx_iis_dev_remove(struct platform_device *pdev)
{
